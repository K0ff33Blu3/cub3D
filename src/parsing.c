...
/*   Updated: 2026/02/05 13:05:53 by miricci          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_rgb(char *str, int *rgb, bool *is_set, int *hex)
{
	...
	while (values[i])
		i++;
	if (i != 3 || !check_rgb_format(values[0])
		|| !check_rgb_format(values[1]) || !check_rgb_format(values[2]))
...
int	floor_ceiling(char *line, t_map *m_map)
{
	...
	if (ft_strncmp(line, "F ", 2) == 0)
		return (parse_rgb(&line[i], m_map->floor_rgb,
				&m_map->floor_set, &m_map->floor_hex));
	else if (ft_strncmp(line, "C ", 2) == 0)
		return (parse_rgb(&line[i], m_map->ceiling_rgb,
				&m_map->ceiling_set, &m_map->ceiling_hex));
	return (0);
}
